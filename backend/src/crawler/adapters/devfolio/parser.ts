...
export function parseDevfolioPayload(html: string): RawHackathon[] {
  ...
  $('a[href]').each((_, element) => {
    ...
    items.push({
      title: title.length > 80 ? title.slice(0, 77) + '...' : title || 'Devfolio hackathon',
      description: `Devfolio event from ${normalizedUrl}`,
...
function looksLikeHackathon(title: string, url: string): boolean {
  ...
  return lowerUrl.includes('devfolio.co') && (lowerTitle.includes('hack') || lowerUrl.includes('/hackathons/') || lowerUrl.includes('/hackathon/'));
}
